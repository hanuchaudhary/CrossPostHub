...
export function CreatePostForm() {
  ...
  const handlePublishPost = async () => {
        ...
        description: (
          <div>
            ...
            <div className="text-xs">
              ...
              <span className="text-neutral-500 text-xs">{new Date().toLocaleDateString()}</span>
              <p className="font-ClashDisplayMedium text-right pt-3 tracking-tighter text-emerald-500">CrossPostHub.</p>
            </div>
          </div>
        ),
      })
    ...
    try {
      ...
      if (isScheduled && scheduleDate && scheduleTime) {
        const scheduledDateTime = new Date(`${format(scheduleDate, "yyyy-MM-dd")}T${scheduleTime}`).toISOString()
      ...
      toast({
        ...
        description: (
          <div className="w-full">
            <Badge className="my-2" variant={isScheduled ? "pending" : "success"}>
      ...
      toast({
        ...
        description: (
          <div className="w-full">
            ...
            <div className="text-xs">
              <p>{error.response?.data?.error || "An error occurred while creating the post."}</p>
      ...
      <Card className="w-full border-none shadow-none md:mx-auto">
        <CardContent className="p-6">
            ...
            <TabsContent value="edit" className="space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <h2 className="text-lg font-ClashDisplayMedium leading-none">Post Content</h2>
                  <AIAssist onGenerate={handleAIAssist} />
                </div>
                <Textarea placeholder="What's on your mind?" value={content} onChange={handleContentChange} rows={5} />
              </div>
              <ImageUpload onChange={handleImageChange} />
              <PlatformSelector selectedPlatforms={selectedPlatforms} setSelectedPlatforms={setSelectedPlatforms} />
              <div className="flex items-center space-x-2">
                <label htmlFor="schedule" className="text-lg font-ClashDisplayMedium leading-none">
                  Schedule this post
                </label>
                <Checkbox
                  id="schedule"
          ...
          <div className="flex justify-end space-x-2 mt-4">
            <Button disabled={isLoading} variant={"customOne"} className="rounded-full" onClick={handlePublishPost}>
